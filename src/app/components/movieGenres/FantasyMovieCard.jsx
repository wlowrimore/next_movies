...
const FantasyMovieCard = ({ fantasyMovies, pageNumber, setPageNumber }) => {
  const [modalData, setModalData] = useState(null);

  const openModal = (fantasyMovie) => {
    setModalData(fantasyMovie);
  }
  ...
  const handleNextPage = () => {
    setPageNumber((prevPage) => {
      const newPage = prevPage < fantasyMovies.total_pages ? prevPage + 1 : fantasyMovies.total_pages;
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return newPage;
    });
  };
  ...
  return (
    <>
      <div className='my-24'>
        <h1 className='text-3xl font-bold mt-6 text-neutral-400'>Fantasy</h1>
        <div className='grid grid-cols-5 gap-10 py-8'>
          {fantasyMovies?.results && fantasyMovies?.results.map((movie) => (
        ...
        <div className='flex items-center justify-center bg-neutral-950 text-white px-4 py-1 mb-[-4rem] rounded-xl'>
          ...
          <p className='mx-10'>Page <span className='text-amber-500'>{pageNumber?.toLocaleString()}</span> of <span className='text-blue-400'>{fantasyMovies?.total_pages.toLocaleString()}</span></p>
          <button
