    ...
    <>
        ...
        <div className='grid grid-cols-5 gap-10 py-8'>
          {actionMovies?.results && actionMovies?.results?.map((movie) => (
            <div key={movie.id} className='max-w-[16rem]'>
              ...
              />
              <h2 className='font-semibold pt-3 text-neutral-400 tracking-wider'>{movie.title}<span className='text-sm font-normal ml-3'>{movie.release_date.split('-')[0]}</span></h2>
              <div className='flex'>
      ...
      {modalData && (
        <div
          ...
          onClick={closeModal}
        >
          <div className="bg-neutral-200 p-4 rounded-lg w-[60%] m-12">
