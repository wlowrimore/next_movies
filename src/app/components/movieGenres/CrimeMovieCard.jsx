...
const CrimeMovieCard = ({ crimeMovies, pageNumber, setPageNumber }) => {
  const [modalData, setModalData] = useState(null);

  const openModal = (crimeMovie) => {
    setModalData(crimeMovie);
  }
  ...
  const handleNextPage = () => {
    setPageNumber((prevPage) => {
      const newPage = prevPage < crimeMovies.total_pages ? prevPage + 1 : crimeMovies.total_pages;
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return newPage;
    });
  };
  ...
  return (
    <>
      <div className='my-24'>
        <h1 className='text-3xl font-bold mt-6 text-neutral-400'>Crime</h1>
        <div className='grid grid-cols-5 gap-10 py-8'>
          {crimeMovies?.results && crimeMovies?.results.map((movie) => (
            <div key={movie.id} className='max-w-[16rem]'>
              ...
              <div className='flex items-center gap-2'>
                <Image
                  ...
                  className='w-6'
                />
                <p classsName='text-neutral-400'>{movie.vote_average.toFixed(1)}&nbsp;<span>&#40;{movie.vote_count}&#41;</span></p>
              </div>
        ...
        <div className='flex items-center justify-center bg-neutral-950 text-white px-4 py-1 mb-[-4rem] rounded-xl'>
          ...
          <p className='mx-10'>Page <span className='text-amber-500'>{pageNumber?.toLocaleString()}</span> of <span className='text-blue-400'>{crimeMovies?.total_pages.toLocaleString()}</span></p>
          <button
