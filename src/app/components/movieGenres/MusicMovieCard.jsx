...
const MusicMovieCard = ({ musicMovies, pageNumber, setPageNumber }) => {
  const [modalData, setModalData] = useState(null);

  const openModal = (musicMovie) => {
    setModalData(musicMovie);
  }
  ...
  const handleNextPage = () => {
    setPageNumber((prevPage) => {
      const newPage = prevPage < musicMovies.total_pages ? prevPage + 1 : musicMovies.total_pages;
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return newPage;
    });
  };
  ...
  return (
    <>
      <div className='my-24'>
        <h1 className='text-3xl font-bold mt-6 text-neutral-400'>Music</h1>
        <div className='grid grid-cols-5 gap-10 py-8'>
          {musicMovies?.results && musicMovies?.results.map((movie) => (
        ...
        <div className='flex items-center justify-center bg-neutral-950 text-white px-4 py-1 mb-[-4rem] rounded-xl'>
          ...
          <p className='mx-10'>Page <span className='text-amber-500'>{pageNumber?.toLocaleString()}</span> of <span className='text-blue-400'>{musicMovies?.total_pages.toLocaleString()}</span></p>
          <button
